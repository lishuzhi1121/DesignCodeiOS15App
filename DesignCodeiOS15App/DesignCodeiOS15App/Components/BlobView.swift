...
struct BlobView: View {
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            // -1.5*60 ~ 1.5*60
            let degress = now.remainder(dividingBy: 3) * 60
            let angle = Angle(degrees: degress)
            let x = cos(angle.radians) // 0 ~ 1.0
            // -3.0*10 ~ 3.0*10
            let degress2 = now.remainder(dividingBy: 6) * 10
            let angle2 = Angle(degrees: degress2)
            let x2 = cos(angle2.radians) // 0 ~ 1.0
